{
  description = "Example project Nix Flake";
  ...
  outputs = inputs@{ self, nixpkgs, flake-utils, ... }: flake-utils.lib.eachDefaultSystem (system:
    ...
    in {
      ...
      packages.example_eth = builtProject;
      packages.reactFrontend = reactFrontend;
      packages.default = self.packages.${system}.example_eth;
    }
  );
}
